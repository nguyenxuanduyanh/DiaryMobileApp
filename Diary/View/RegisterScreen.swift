            ...
            VStack(alignment: .leading){
                ...
                HStack {
                    ...
                    VStack(alignment: .leading) {
                        ...
                        TextField("User name", text: $registerController.username).font(.system(size: 22))
                        Spacer().frame(height: 24)
                        Text("EMAIL").font(.system(size: 24)).fontWeight(/*@START_MENU_TOKEN@*/.bold/*@END_MENU_TOKEN@*/)
                        Spacer().frame(height: 12)
                        TextField("Email address", text: $registerController.email).font(.system(size: 22)).keyboardType(.emailAddress)
                        Spacer().frame(height: 24)
                        Text("PASSWORD").font(.system(size: 24)).fontWeight(/*@START_MENU_TOKEN@*/.bold/*@END_MENU_TOKEN@*/)
                        Spacer().frame(height: 12)
                        ZStack(alignment: Alignment(horizontal: .trailing, vertical: .center)) {
                            if !showPassword {
                                SecureField("Your password", text: $registerController.password).font(.system(size: 22))
                            } else {
                                TextField("Your password", text: $registerController.password).font(.system(size: 22))
                            }
                            Button(action: {
                        ...
                        ZStack(alignment: Alignment(horizontal: .trailing, vertical: .center)) {
                            if !showRePassword {
                                SecureField("Confirm your password", text: $registerController.confirmPassword).font(.system(size: 22))
                            } else {
                                TextField("Confirm your password", text: $registerController.confirmPassword).font(.system(size: 22))
                            }
                            Button(action: {
